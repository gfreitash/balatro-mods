...
get_latest_version_info() {
    ...
    version_line_info=$(grep -m 1 -n -E "## \[[0-9]+\.[0-9]+\.[0-9]+\]" "$changelog_file" 2>/dev/null || true)

    if [ -n "$version_line_info" ]; then
        LATEST_VERSION_LINE_NUM=$(echo "$version_line_info" | cut -d: -f1)
        LATEST_VERSION=$(echo "$version_line_info" | grep -oP '\[\K[0-9]+\.[0-9]+\.[0-9]+(?=\])' 2>/dev/null || true)
...
content_exists_in_range() {
    ...
    if [ -z "$start_line" ] || [ -z "$end_line_exclusive" ] || [ "$start_line" -ge "$end_line_exclusive" ]; then
    ...
    extracted_content=$(sed -n "${start_line},${actual_end_line}p" "$file" 2>/dev/null | tr -d '\r')
...
insert_content_after_line() {
    ...
    # FIX: Removed comments from inside the awk script block
...
for mod_dir in $MOD_DIRS; do
    ...
    if echo "$CHANGED_FILES" | grep -q -E "^${mod_name}/"; then
        ...
        unreleased_content=$(sed -n '/## \[Unreleased\]/,/^## \[/p' "$changelog_file" | grep -v -e '##' -e '^[[:space:]]*$' || true)
...
for mod_dir in $MOD_DIRS; do
    ...
    if [ "$current_manifest_version" != "$LATEST_VERSION" ]; then
        ...
        jq --arg dep "$SHARED_DEP_REQ" '.dependencies = [$dep]' "$manifest_file" > "${manifest_file}.$$" && mv "${manifest_file}.$$" "$manifest_file"
