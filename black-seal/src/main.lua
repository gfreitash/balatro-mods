...
RIOSODU_SHARED.utils.sendDebugMessage("Main logic module loading...")
...
function BSM.utils.calculate_and_set_seal_weights()
  RIOSODU_SHARED.utils.sendDebugMessage("Calculating and setting seal weights...")
  ...
  for key, seal_data in pairs(G.P_SEALS) do
      RIOSODU_SHARED.utils.sendDebugMessage(string.format("G.P_SEALS[%s].weight = %.2f (original weight)", key, seal_data.weight or 5))
      local new_weight = 0
      if key == BSM.black_seal_id_full then
      ...
      RIOSODU_SHARED.utils.sendDebugMessage(string.format("G.P_SEALS[%s].weight = %.2f (new weight)", key, new_weight))
    ...
    if not G.P_SEALS[BSM.black_seal_id_full] then
        RIOSODU_SHARED.utils.sendDebugMessage(string.format("G.P_SEALS[%s].weight = %.2f (initial weight)", BSM.black_seal_id_full, percentage))
    end
  ...
  calculate = function(self, card, context)
    ...
    G.E_MANAGER:add_event(Event({
      func = function()
        ...
        RIOSODU_SHARED.utils.sendDebugMessage("Black Seal removal event running...")
        ...
        RIOSODU_SHARED.utils.sendDebugMessage("Finished checking and cleaning Black Seals.")
        return true
      end
    }))
    return {}
